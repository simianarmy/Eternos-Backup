...
describe "backup worker" do
  ...
  describe "linkedin" do
    ...
    def validate_user_associations(li_user)
      li_user.linkedin_user_connections.should_not be_empty
      li_user.linkedin_user_positions.should_not be_empty
      li_user.linkedin_user_educations.should_not be_empty
      #li_user.linkedin_user_past_positions.should_not be_empty
      li_user.linkedin_user_current_positions.should_not be_empty
    end
    ...
    describe "with valid credentials" do
      ...
      describe "on the first backup" do
        before(:each) do
          @backup_source.stubs(:linkedin_user).returns(nil)
        end
      
        it "should create a linkedin user record" do
          lambda {
            do_backup @backup_source
          }.should change(LinkedinUser, :count).by(1)
        end
        
        it "should add the profile data" do
          do_backup @backup_source
          @profile = LinkedinUser.backup_source_id_eq(@backup_source.id).first
        ...
        it "should add associations" do
          do_backup @backup_source
          @profile = LinkedinUser.backup_source_id_eq(@backup_source.id).first
          validate_user_associations @profile.reload
        end
      end
