...
describe BackupWorker::Base do
  ...
  before(:each) do 
    @worker = BackupWorker::TestWorker.new(@job = create_backup_job)
  end
  
  describe "on create" do
    it "should be a child of BackupWorker::Base" do 
      @worker.should be_a BackupWorker::Base
    end
    
    it "should use a default increment step of 100 when no actions defined" do
      @worker.actions.should be_empty
      @worker.send(:increment_step).should == 100
    end
  ...
  describe "on run" do
    it "should call each method in actions array using save_ prefix" do
      @worker.actions = {EternosBackup::SiteData.defaultDataSet => [:foome, :fooyou]}
