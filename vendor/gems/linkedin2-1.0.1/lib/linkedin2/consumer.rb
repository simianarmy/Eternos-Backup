...
module Linkedin2
  class Consumer
    ...
    # Checks if data access is allowed for this consumer
    def authorized?
      name = get_first_name
      !name.nil? && !name.blank?
    end
    ...
    def get_network_update(type)
      if (type.upcase == 'NCON')
        ...
        if hash && hash['updates'] && hash['updates']['update']
          hash['updates']['update'].delete_if do |update|
